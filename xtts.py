...
def load_audio(audiopath, sampling_rate):
    ...
    # Check some assumptions about audio range.
...
class Xtts(BaseTTS):
        ...
        for sent in text:
            ...
            with torch.no_grad():
                ...
                gpt_latents = self.gpt(
                    ...
                    wav_lengths=expected_output_len,
                    cond_mels=None,
                    cond_latents=gpt_cond_latent,
    ...
    def forward(self, batch: dict) -> dict:
        """
        Forward pass for XTTS during training.
        Accepts a batch dict with keys like token_id, token_id_lengths, gpt_audio_tokens, etc.
        """

        # Optional safety check
        required_keys = ["token_id", "token_id_lengths", "gpt_audio_tokens",
                        "expected_output_len", "gpt_cond_latent", "speaker_embedding"]
        for k in required_keys:
            if k not in batch:
                raise ValueError(f"Batch is missing key '{k}'")

        # Map batch entries to local variables
        # Debug: see what keys exist
        print("DEBUG batch keys:", batch.keys())

        text_inputs = batch.get("token_id")
        if text_inputs is None:
            raise KeyError(f"'token_id' missing in batch! Available keys: {list(batch.keys())}")

        # ensure it's a tensor
        if isinstance(text_inputs, list):
            text_inputs = torch.nn.utils.rnn.pad_sequence([t if isinstance(t, torch.Tensor) else torch.tensor(t, dtype=torch.long) for t in text_inputs], batch_first=True)

        text_inputs = text_inputs.to(self.device)

        text_lengths = batch.get("token_id_lengths").to(self.device)
        gpt_audio_tokens = batch.get("gpt_audio_tokens").to(self.device)
        expected_output_len = batch.get("expected_output_len").to(self.device)
        gpt_cond_latent = batch.get("gpt_cond_latent").to(self.device)
        speaker_embedding = batch.get("speaker_embedding").to(self.device)

        # ... rest of forward pass as before

        # Forward pass through autoregressive GPT
        gpt_latents = self.gpt(
            text_inputs=text_inputs,
            text_lengths=text_lengths,
            audio_codes=gpt_audio_tokens,
            wav_lengths=expected_output_len,
            cond_mels=batch["cond_mels"],                       # keep
            cond_latents=batch.get("gpt_cond_latent", None),    # safer for batch inference
            return_attentions=False,
        )

        # Forward pass through HifiGAN decoder
        waveform = self.hifigan_decoder(gpt_latents, g=speaker_embedding)
    ...
    def get_criterion(self):
        # XTTS uses a custom trainer; for compatibility with generic Trainer we provide a dummy loss
        def dummy_loss(*args, **kwargs):
            return torch.tensor(0.0, requires_grad=True).to(self.device)
        return dummy_loss

    def train_step(self, batch, criterion):
        # Map DataLoader batch to forward() expected keys
        forward_batch = {
            "token_id": batch["token_id"].to(self.device),
            "token_id_lengths": batch["token_id_lengths"].to(self.device),
            "gpt_audio_tokens": batch["gpt_audio_tokens"].to(self.device),
            "expected_output_len": batch["expected_output_len"].to(self.device),
            "gpt_cond_latent": batch["gpt_cond_latent"].to(self.device),
            "speaker_embedding": batch["speaker_embedding"].to(self.device),
        }

        # Call forward with the properly structured batch
        model_outputs = self.forward(forward_batch)

        # Compute loss
        loss_dict = criterion(model_outputs, forward_batch)
